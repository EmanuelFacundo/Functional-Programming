...
createCar p g = Info (Car 0 p 0 g 0) (Op "create" True)

-- enche o tanque passando a qtd de gas. Retorna falso apenas se o tanque já estiver completamente cheio.
fuel :: Int -> Info -> Info
fuel f (Info (Car pass maxPass gas maxGas km) (Op name result)) 
  | gas == maxGas = Info (Car pass maxPass gas maxGas km) (Op "fuel" False)
  | gas + f >  maxGas = Info (Car pass maxPass maxGas maxGas km) (Op "fuel" True)
  | otherwise = Info (Car pass maxPass (gas + f) maxGas km) (Op "fuel" True)

-- Faz entrar uma pessoa no carro. Retorna false se já estiver lotado.
embark :: Info -> Info 
embark (Info (Car pass maxPass gas maxGas km) (Op name result)) = 
  if pass + 1 > maxPass
    then Info (Car pass maxPass gas maxGas km) (Op "embark" False)
  else
    Info (Car (pass + 1) maxPass gas maxGas km) (Op "embark" True )

-- Retira uma pessoa do carro, retorna false se não tiver ninguém no carro
disembark :: Info -> Info
disembark (Info (Car pass maxPass gas maxGas km) (Op name result)) = 
  if pass - 1 < 0 
    then Info (Car pass maxPass gas maxGas km) (Op "disembark" False)
  else
    Info (Car (pass - 1) maxPass gas maxGas km) (Op "disembark" result)
...
drive :: Int -> Info -> Info 
drive d (Info (Car pass maxPass gas maxGas km) (Op name result))  
  | gas == 0 && d /= 0 = Info (Car pass maxPass gas maxGas km) (Op "drive" False)
  | (gas - d) < 0 = Info (Car pass maxPass 0 maxGas (km + gas)) (Op "drive" False)
  | otherwise = Info (Car pass maxPass (gas - d) maxGas (km + d)) (Op "drive" True)

-- Testes
test1 :: IO ()
test1 = print $ resume . embark . resume. embark . resume $ createCar 2 50
...
Info {car = Car {pass = 2, maxPass = 2, gas = 0, maxGas = 50, km = 0}, op = Op {name = "embark", result = True}}
-}

-- Resultado segundo teste
{-
Car pass: 0/2 gas: 0/50 km: 0 Operation: create Result: True
Car pass: 1/2 gas: 0/50 km: 0 Operation: embark Result: True
